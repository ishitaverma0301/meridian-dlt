...
- **Non-destructive data quality** — all 16 validation rules run in a single pass, producing one `rejection_reason` column per row. Failing rows are persisted for audit, never silently dropped; only the clean, deduplicated set feeds Gold.
...
│  (scheduled job) │     │                                                │
│                  │     │   Bronze          Silver            Gold      │
│  Source ZIPs ────┼────▶│  streaming   ──▶  16 DQ rules  ──▶  8 tables  │
│  → raw Volume    │     │  table via        + dedup on         + trend  │
│                  │     │  Auto Loader       reprocess         (SQL,    │
│                  │     │                    → clean /         Volume- │
│                  │     │                      rejected        sourced)│
└─────────────────┘     └──────────────────────────────────────────────┘
        │                                    │
...
**Silver** computes a single `rejection_reason` expression across all 16 rules in one pass, then splits into a clean view and a rejected view. The same computation deduplicates Bronze down to the latest ingestion per month before evaluating any rule, so the two-versions-of-one-month situation from Bronze never reaches Gold.
...
All 16 rules execute in a single Spark SQL pass; a row can report multiple violations at once, joined into one `rejection_reason` string. Failing rows are persisted, never dropped.
...
| **16 Data Quality Rules** | Single-pass evaluation, non-destructive rejection, full audit trail |
...
│   └── pipeline/
│       ├── 01_bronze.py                 # Bronze: Auto Loader streaming table
│       ├── 02_silver.py                 # Silver: 16 DQ rules, dedup, clean/rejected split
│       ├── 03_gold.py                   # Gold: loads & registers 8 tables from SQL artifacts
│       └── gold/                        # The 8 externalized SQL artifacts themselves
